    ...
    def calc(self) :
        for self.stopt in range(1,self.tmax) :
            ...
            if max(self.f) > self.fbest :
                ...
                self.xbest = copy.copy(self.x[np.argmax(self.f)]) # xbestの更新
...
if __name__ == '__main__' :
    ...
    weight = np.array([3,6,5,4,8,5,3,4,8,2]) # 品物10個
    # value = np.array([50,40,10,70,55]) # 品物5個
    ...
    for i in range(100) :
        ...
        # 最適解は?
        if suit_value < ga1.fbest :
            suit_value = ga1.fbest

        # 最適解を何回求めるか
        # if int(ga1.fbest) == 125 : # 品物5個
        print(ga1.fbest)
        if int(ga1.fbest) == 420 : # 品物10個
            cnt += 1
            if suit_flag :
                xb = ga1.xbest
                suit_flag = False
    ...
    print(f"最適値 Fg = {suit_value}")
    print(f'最適解 Xbest = {xb}')